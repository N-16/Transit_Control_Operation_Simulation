...
class Agent():
    def __init__(self, gamma:float, epsilon:float, alpha:float,
                n_actions:int, input_dims:tuple, mem_size:int, batch_size:int,
                eps_min:float, eps_decay:float, replace = 1000, chkpt_dir = 'models/single_agent_models_to_load'):
        self.gamma = gamma
        self.epsilon = epsilon
        self.eps_min = eps_min
        self.eps_dec = eps_decay
    ...
    def save_models(self, name='single'):
        self.q_eval.save_checkpoint(name)
        self.q_next.save_checkpoint(name)

    def load_models(self,name='single'):
        self.q_eval.load_checkpoint(name)
        self.q_next.load_checkpoint(name)
